/* contact.js — WhatsApp & Telegram (с поддержкой бота и жёсткой валидацией)
   ...
   Telegram:   если задан и bot, и bot_prestart_url -> POST /prestart -> t.me/<bot>?start=<token>
               иначе — копируем текст в буфер и открываем чат (бот или @username).

   Требования к конфигу (пример):
   <script>
     window.APP_CONFIG = {
...
(function () {
  ...
  // Нормализуем WA сразу: только цифры в формате E.164
  const WA_DIGITS = digits(CFG.whatsapp || DEF_WA);

  // Нормализуем bot_prestart_url: если прислан /health — меняем на /prestart
  function normalizePreUrl(u) {
    ...
    try {
      ...
      // Если это не валидный URL — вернём как есть (пусть упадёт явно)
      return u;
    }
  }
  ...
  function toast(msg) {
    ...
    el.textContent = msg;
    el.style.cssText =
      'position:fixed;left:50%;transform:translateX(-50%);bottom:120px;background:#0D2B1E;color:#fff;padding:8px 12px;border-radius:10px;font:500 13px system-ui,-apple-system,Segoe UI,Roboto,sans-serif;box-shadow:0 10px 25px rgba(0,0,0,.2);z-index:10000;opacity:0;transition:opacity .2s';
  ...
  function validateForm(form) {
    ...
    if (!fName || !fName.value.trim()) {
      showError(fName || form, t('required'));
      ok = false;
    }

    if (!fContact || !fContact.value.trim()) {
      showError(fContact || form, t('required'));
      ok = false;
    } else if (!(isPhone(fContact.value) || isTG(fContact.value))) {
      showError(fContact, t('badContact'));
      ok = false;
    }

    if (!fDate || !fDate.value) {
      showError(fDate || form, t('badDate'));
      ok = false;
    }

    const g = parseInt((fGuests && fGuests.value) || '0', 10);
    const minG = Math.max(1, Number(fGuests?.min || 1));
    if (!Number.isFinite(g) || g < minG) {
      showError(fGuests || form, t('badGuests'));
      ok = false;
    }

    if (!fMsg || !fMsg.value.trim()) {
      showError(fMsg || form, t('required'));
      ok = false;
    }
  ...
  function programInfo(fromEl) {
    const root =
      fromEl?.closest('[data-program],[data-program-title],[data-program-id]') || document.body;
    ...
    const title =
      root?.dataset?.programTitle || metaTitle || heroTitle || docTitle || 'Программа';
    const id =
  ...
  function openDeepLink(deep, web) {
    if (isMobile) {
      ...
      let w;
      try {
        w = window.open(deep, '_blank');
      } catch {}
      setTimeout(() => {
        try {
          if (!w || w.closed) window.open(web, '_blank', 'noopener,noreferrer');
        } catch {
  ...
  // Telegram через бота (если настроен), иначе фолбэк
  async function openTG(text, ctx, prog) {
    ...
    if (TG_BOT && PRE_URL) {
      ...
      try {
        ...
        // Пытаемся прочитать JSON даже при ошибке статуса — покажем причину в консоли
        let j = null;
        try { j = await r.json(); } catch {}

        if (!r.ok || !j?.ok || !j?.token) {
          console.warn('[contact.js] prestart bad response:', { status: r.status, body: j });
          throw new Error('Bad token/response');
        }

        const token = encodeURIComponent(j.token);
        const deep = `tg://resolve?domain=${TG_BOT}&start=${token}`;
        const web = `https://t.me/${TG_BOT}?start=${token}`;
    ...
    // Фолбэк: копируем текст и открываем чат (бот, если указан; иначе — username)
  ...
  document.addEventListener(
    ...
    (e) => {
      const a = e.target.closest(
        'a[data-whatsapp], a[data-telegram], [data-book], [data-booking], .js-book, a[href^="#book"]'
      );
      if (!a) return;
      ...
      if (a.matches('[data-book], [data-booking], .js-book, a[href^="#book"]')) {
        ...
        const chSel = (
          ...
          a.dataset.channel ||
          a.closest('[data-channel]')?.dataset.channel ||
          ''
        ).toLowerCase();
  ...
  if (document.readyState === 'loading')
    document.addEventListener('DOMContentLoaded', patchBase, { once: true });
  else patchBase();

  // Немного отладки наружу (по желанию можно удалить)
  ...
  console.log('[contact.js] ready (v2) — validation + WA normalize + TG bot/prestart (+timeout) + fallbacks');
})();
