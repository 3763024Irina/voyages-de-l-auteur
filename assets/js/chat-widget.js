/* chat-widget.js — FAB WA + TG, v1.3-clean
   — БЕЗ копирования и тостов
   — TG: бот через /prestart → https://t.me/<bot>?start=<token>
   — TG profile: https://t.me/<user>
   — WA: wa.me с текстом заказа
*/
;(function(){
...
— Пожелания: ____`;
  ...
  async function openTelegramBot(){
    ...
    window.location.href = `https://t.me/${TG_BOT}?start=${encodeURIComponent(token)}`;
    setTimeout(()=>{ tgLock=false; }, 600);
  }
  ...
  function openTelegramProfile(){
    ...
    if (!who) return;
    window.location.href = `https://t.me/${who}`;
  }
  ...
  menu.innerHTML = `
    <div class="chat-row">
      ...
      <button class="chat-ico" type="button" data-a="tg" aria-label="Telegram">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.6 15.3 9.3 20c.5 0 .8-.2 1-.4l2.4-2.3 5-3.7c.9-.6 1.5-1.1 1.7-1.8.3-.6-.1-1-.9-.8l-6.8 2.6-2.8.9c-.6.2-1 .1-1.3 0-.3-.2-.3-.6.2-.9л4.5-2.2 6.1-3.2c.7-.3 1.2-.6 1.2-1 0-.3-.5-.4-1.2-.2l-10.6 4c-1 .4-1.7.9-1.7 1.5s.6.9 1.5 1l2 .3 7.8 4.9c.8.5 1.5.2 1.7-.7" fill="currentColor"/></svg>
      </button>
    </div>`;
